...
namespace Gsstwpfmock.ViewModel
{
    public partial class MainWindowViewModel : ObservableObject
    {
        ...
        public void Initalize()
        {
            ...
            IsAdmin = _userSession.SessionUser.Role == UserRoles.Admin || _userSession.SessionUser.Role == UserRoles.Manager;
        ...
        public void Checkout()
        {
            ...
            try
            {
                var paymentService = Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<IPaymentService>(_serviceProvider);
                var authService = Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<IAuthService>(_serviceProvider);
                var userService = Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<IUserService>(_serviceProvider);

                var paymentViewModel = new PaymentViewModel(paymentService, authService, userService, _bonusService, CurrentOrder.Total, BonusCard);
                var paymentWindow = new PetrolWPF.View.PaymentWindow(paymentViewModel);

                if (paymentWindow.ShowDialog() == true)
                {
                    CurrentOrder.PaymentCardNumber = paymentWindow.FinalPaymentDetails;
                    CurrentOrder.PaymentStatus = "Paid";
                    CurrentOrder.BonusSpent = paymentViewModel.UseBonuses ? paymentViewModel.BonusesToSpend : 0;
                    CurrentOrder.BonusCardId = paymentViewModel.ClientBonusCard?.Id;

                    int currentUserId = paymentViewModel.ClientUser?.Id ?? _userSession.SessionUser.Id;

                    _orderService.ProcessCheckout(CurrentOrder, currentUserId);

                    string receipt = $"--- RECEIPT ---\n" +
                                     $"Status: PAID\n" +
                                     $"Method: {paymentWindow.FinalPaymentDetails}\n" +
                                     $"Subtotal: {CurrentOrder.Total + CurrentOrder.BonusSpent:F2} $\n" +
                                     $"Bonuses Spent: -{CurrentOrder.BonusSpent} $\n" +
                                     $"Total Paid: {CurrentOrder.Total:F2} $\n\n" +
                                     $"Thank you for your purchase!";

                    MessageBox.Show(receipt, "Payment Successful", MessageBoxButton.OK, MessageBoxImage.Information);
